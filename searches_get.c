...
int file_search_datazone_get(int zonesize,
...
int search_all_get(int fd,
    ...
    int ret = search_direct_get(fd, inode, config, inode_start, zonesize, final_dir);
    ...
    if(ret == 1){
        ...
        if(read_inode_get(fd, final_inode, inode_start, final_dir->inode) == -1){
...
int search_direct_get(int fd, 
    ...
    int path_depth = strtok_count_get(config->path);
    int local_count = 0;
    int zone_off = 0;
    char* target = NULL;
    char* delim = "/";
    ...
    while((target = strtok(config->path, delim)) != NULL){
        ...
        for(zone_list_idx = 0; zone_list_idx < DIRECT_ZONES; zone_list_idx++){
            ...
            //read the data zones
            if(read_zone_get(fd, zone_off, zonesize, zone_data) == -1){
                fprintf(stderr, "read_zone error\n");
                return 1;
            }
            //want to search for target of current inode
            int ret = 0;
            if((ret = file_search_datazone_get(zonesize, zone_data, (unsigned char*)target, cur_dir)) == 1){
                local_count++;
            ...
            if((read_inode_get(fd, cur_inode, inode_start, cur_dir->inode)) == -1){
