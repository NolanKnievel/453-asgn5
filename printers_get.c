...
int print_macros_dir_get(int fd, uint16_t firstdata, 
        struct inode* inode, int zonesize, 
        off_t inode_start, off_t data_start){
    //locals
    ...
    struct directory* zone_data = 
        (struct directory*)malloc(sizeof(struct directory) * dir_per_zone);
    ...
    for(zone_list_idx = 0; zone_list_idx < DIRECT_ZONES; zone_list_idx++){
        ...
        //calculate offset 
        offset = calc_datazone_addr_get(data_start, firstdata,
             zonesize, inode->zone[zone_list_idx]);

        //read each zone
        if(read_zone_get(fd, offset, zonesize, zone_data) == -1){
        ...
        for(dir_idx = 0; dir_idx < dir_per_zone; dir_idx++){
            ...
            *cur_dir = zone_data[dir_idx];
            //get inode
            if(read_inode_get(fd, cur_inode, 
                    inode_start, cur_dir->inode) == -1){
            ...
            //---
            print_macros_file_get(cur_dir, cur_inode);
        }
    }
    printf("\n");
    free(cur_inode);
...
int print_macros_file_get(struct directory* dir, struct inode* parent){
    if(print_permissions_get(parent) == -1){
        fprintf(stderr, "print_file perm error\n");
        return -1;
    }
    if(print_inum_get(dir->inode) == -1){
        fprintf(stderr, "print_file inum error\n");
        return -1;
    }
    if(print_name_get(dir->name) == -1){
